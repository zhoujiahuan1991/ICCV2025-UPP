...
from tools.runner_finetune import Gaussian_noise_number, lidar_noise_number
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    shutil.copy('models/Point_MAE_unify.py', args.experiment_path)
    ...
    if args.resume:
        ...
        if args.ckpts is not None:
            base_model.load_model_from_ckpt(args.ckpts, logger=None)
        else:
    ...
    peft_list = ['downstream_adapter', 
                 ...
                 'downstream_prompts',
                 'bnorm',
                 'cls_pos', 'cls_token', 'cls_head_finetune']
    ...
    metrics = validate(base_model, test_dataloader, 0, val_writer, args, config, logger=logger, pointr_model=None)
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for batch_idx, (taxonomy_ids, model_ids, data) in enumerate(tqdm(train_dataloader)):
            ...
            ret = base_model(points, completion_prompt=config.noisy_train, denoise=config.noisy_train, point_num=npoints, batch_idx=batch_idx)
        ...
        if args.joint_optimization == epoch:
            print_log("Additional require gradient parameters: ", logger = logger)
            peft_list = ['downstream_adapter', 'downstream_adapter1', 
                     'downstream_prompts',
                     'dense_pred', 'mask_token', 
                     'rectify_prompter', 'shape_pred',
                     'coarse_pred', 'mask_token', 'dense_pred',
                     'rectify_prompter', 'shape_pred', 'coarse_pred', 'predict_token_generator', 
            ...
            for name, param in base_model.named_parameters():
                if misc.peft_detect(name, peft_list) : 
                    ...
                    param.requires_grad_(False)
            # from utils.misc import summary_parameters
            # summary_parameters(base_model, logger=logger, in_detail=False)
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            metrics = validate(base_model, test_dataloader, epoch, val_writer, args, config, logger=logger, pointr_model=None)
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger = None, pointr_model=None):
    ...
    with torch.no_grad():
        for batch_idx, (taxonomy_ids, model_ids, data) in enumerate(tqdm(test_dataloader)):
            ...
            #     # Lidar noise
            #     B, P, C = points.shape
            #     lidar_noise = misc.lidar_noise(points, 20, low=1.3, scale=1.8,) #deterministic=True
            #     points = torch.concat([points, lidar_noise], dim=1)
            #     # Gaussian noise
            #     # B, P, C = points.shape
            ...
            if pointr_model is not None:
                ...
                if visualize == True:
                    ...
                    np.save(f'./visualization/{task}/coarse-{batch_idx}', coarse_points.detach().cpu().numpy())
                    np.save(f'./visualization/{task}/dense-{batch_idx}', dense_points.detach().cpu().numpy())
                    np.save(f'./visualization/{task}/points-{batch_idx}', points.detach().cpu().numpy())
                points = dense_points
            logits = base_model(points, batch_idx=batch_idx, completion_prompt=config.noisy_validate, denoise=config.noisy_validate, point_num=npoints) # config.noisy_validate
            target = label.view(-1)
...
def test(base_model, test_dataloader, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            logits = base_model(points, batch_idx=idx, completion_prompt=config.noisy_validate, denoise=config.noisy_validate, point_num=npoints) # config.noisy_validate
            target = label.view(-1)
