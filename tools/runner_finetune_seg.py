...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # peft_list = ['adapter', 'cls_pos', 'cls_token', 'cls_head_finetune']
    ...
    test_metrics = validate(logger, num_part, classifier, test_dataloader, num_classes, config, noise_test=True)
    print(test_metrics)
    # trainval
    # metrics = validate(classifier, test_dataloader, 0, val_writer, args, config, logger=logger, pointr_model=None, incomplete_cropping=False) # pointr_model
    # training
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for batch_idx, (points, label, target) in enumerate(tqdm(train_dataloader)):
            ...
            seg_pred = classifier(points, cls_label, label_points=gt_points)
            seg_pred = seg_pred.contiguous().view(-1, num_part)
            target = target.view(-1, 1)[:, 0]
            pred_choice = seg_pred.data.max(1)[1]

            acc = pred_choice.eq(target.data).cpu().sum()/(config.total_bs * npoints)
...
def validate(logger, num_part, classifier, testDataLoader, num_classes, config, noise_test=False):
    with torch.no_grad():
        ...
        for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
            ...
            if noise_test:
                B, P, C = points.shape
                Gaussian_noise = misc.gaussian_noise([B, 24, C], loc=0., scale=0.1, shell_radius=0.9)
                Gaussian_noise = torch.tensor(Gaussian_noise, dtype=torch.float32).cuda()
                points  = torch.concat([points, Gaussian_noise], dim=1)
                lidar_noise = misc.lidar_noise(points, 64, low=1.2, scale=1.5)
                points = torch.concat([points, lidar_noise], dim=1)
                gt_points = points[:,:P]
            seg_pred = classifier(points, to_categorical(label, num_classes), label_points=gt_points)
