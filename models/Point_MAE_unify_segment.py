...
import emd 
from .Point_MAE_unify import Group, propagate
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, pos, **kwargs):
        ...
        if kwargs.get('feature_list'):
            feature_list = []
            fetch_idx = [3, 7, 11]
        for idx, block in enumerate(self.blocks):
            if idx == block_depth:
                break
            x = block(x + pos, block_idx=idx, **kwargs)
            if kwargs.get('feature_list'):
                if idx in fetch_idx:
...
class Point_MAE_unify_seg(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[Point_MAE_unify_seg] ', logger ='Point_MAE_unify_seg')
        self.config = config
        ...
        print_log(f'divide point cloud into {self.num_group} x {self.group_size} points ...', logger ='Point_MAE_unify_seg')
        ...
        self.group_divider1 = Group(num_group=self.num_group*4, group_size=self.group_size//2)
        # self.propagation_1 = PointNetFeaturePropagation(in_channel=384*3+75, mlp=[384*2, 384*2], interpolate_neighbors=6)

        self.seg_head = nn.Sequential(
            nn.Conv1d(1024+128+384*6, 512, 1),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Conv1d(512, 256, 1),
    ...
    def forward(self, pts, cls_label, label_points=None, completion_prompt=True, denoise=True, point_num=1024, **kwargs):
        if label_points is not None:
            B, N, _ = label_points.shape
        else:
            B, N, _ = pts.shape
        ...
        if completion_prompt:
            ...
            x_rec = self.MAE_decoder(x_full, pos_full, M, pretask_adapter=True, path='pretask')
        ...
        # # transformer
        feature_list = self.blocks(x, 
                                   pos, 
                                   path='downstream', 
                                   downstream_adapter=self.config.prompter_config.downstream_adapter, 
                                   downstream_prompts=self.config.prompter_config.downstream_prompts, 
        ...
        # x_global_feature = torch.cat((x_max_feature, cls_label_feature), -1) #1152*1 + 64
        torch.cuda.empty_cache()
 
        # _, center1 = self.group_divider1(pts)
        # f_level_1 = self.propagation_1(center1, center, self.positional_embedding(center1), x)  
        if label_points is not None:
            # f_level_0 = self.propagation_0(label_points, center1, self.positional_embedding(label_points), f_level_1)
            f_level_0 = self.propagation_0(label_points, center, label_points, x) 
        else:
            f_level_0 = self.propagation_0(pts, center, pts, x) 
        # if label_points is not None:
        #     f_level_0 = self.propagation_0(label_points, center, self.positional_embedding(label_points), x)
        # else:
        #     f_level_0 = self.propagation_0(pts, center, self.positional_embedding(pts), x)
