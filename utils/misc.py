...
def gaussian_noise(shape: list, loc=0., scale=0.2, shell_radius=0.9, deterministic=False):
    '''produce Shell Noise with Gaussian distribution to imitate noisy points from depth cameras'''
    if deterministic:
        gaussian = np.load("deterministic_noise/gaussian_noise.npy")[:shape[0]]
    else:
...
def lidar_noise(points, number=64, scale=1.3, low=1.02, deterministic=False):
    if deterministic:
        noise_idx = np.load("deterministic_noise/noise_idx.npy")
        noise_factor = np.load("deterministic_noise/noise_factor.npy")
    else:
    ...
    for points in xyz:
        ...
        if isinstance(crop,list):
            ...
            if incomplete_shape and input_data.shape[1] > sample_points:
                ...
                INPUT.append(input_data)
            if incomplete_shape and crop_data.shape[1] > sample_points:
                CROP.append(fps(crop_data,sample_points)[0])
            else:
                CROP.append(crop_data)
